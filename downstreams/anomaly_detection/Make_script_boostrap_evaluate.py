...
def prepare_script(args):
    ...
    train_list = []
    for iboostrap in [args.target_boostrap]:
        ...
        f_predict.write(f'python3 01predict_SR.py {os.path.abspath(config_workflow_file)} --region SR --checkpoint {f"{storedir}/SB/model_checkpoint"} --gen_num_events {args.gen_events * args.num_toys} --ngpu {args.gpu} --kfold {args.k}\n')
        if args.no_signal:
            f_predict.write(f'python3 01predict_SR.py {os.path.abspath(config_workflow_file)} --region SR --checkpoint {f"{no_signal_storedir}/SB/model_checkpoint"} --gen_num_events {args.gen_events * args.num_toys} --ngpu {args.gpu} --kfold {args.k} --no_signal\n')
...
def main():
    ...
    parser.add_argument("--target_boostrap", type = int, default = 1, help="Bootstrap number")
    parser.add_argument("--num_toys", type = int, default = 1000, help="Number of toys to generate")
