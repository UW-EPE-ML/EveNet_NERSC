...
class JetReconstructionOptimization(JetReconstructionNetwork):
    def __init__(self, config: Config, torch_script: bool = False):
        super(JetReconstructionOptimization, self).__init__(config, torch_script)

        self.num_losses = (
                (self.options.Training.assignment_loss_scale > 0) * len(self.event_info.assignment_names) +
                (self.options.detection_loss_scale > 0) * len(self.event_info.assignment_names) +
                (self.options.regression_loss_scale > 0) * len(self.event_info.regression_names) +
                (self.options.classification_loss_scale > 0) * len(self.event_info.classification_names) +
    ...
    def jacobian_loop(outputs: Tensor, inputs: Tuple[Tensor], create_graph: bool = False, strict: bool = False):
        ...
        for i, out in enumerate(outputs):
            ...
            jacobian += (
                tuple(
                    torch.stack(jac_i_el, dim=0).view(out.size() + inputs[el_idx].size())
