...
from evenet.network.layers.linear_block.activations import create_activation, create_dropout
...
class NormalizationFirstBlock(nn.Module):
    ...
    def __init__(
        ...
        self.activation = create_activation(options.Network.linear_activation, output_dim)

        # Optional activation normalization. Either batch or layer norm.
        normalization = options.Network.normalization if normalization is None else normalization
        self.normalization = create_normalization(normalization, output_dim)

        # Optional dropout for regularization.
        self.dropout = create_dropout(options.Network.dropout if dropout is None else dropout)
...
class ResNetBlock(nn.Module):
    def __init__(self, options: DotDict, input_dim: int, output_dim: int, skip_connection: bool = True):
