...
from evenet.network.layers.linear_block.activations import create_activation, create_dropout, create_residual_connection
from evenet.network.layers.linear_block.normalizations import create_normalization
...
class GRUBlock(nn.Module):
    ...
    def __init__(self, options: DotDict, input_dim: int, output_dim: int, skip_connection: bool = False):
        super(GRUBlock, self).__init__()

        self.input_dim = input_dim
        self.output_dim = output_dim
        self.skip_connection = skip_connection
        self.hidden_dim = int(round(options.Network.transformer_dim_scale * input_dim))

        # Create normalization layer for keeping values in good ranges.
        self.normalization = create_normalization(options.Network.normalization, input_dim)

        # The primary linear layers applied before the gate
        self.linear_1 = nn.Sequential(
            nn.Linear(input_dim, self.hidden_dim),
            create_activation(options.Network.linear_activation, self.hidden_dim),
            create_dropout(options.Network.dropout)
        )

        self.linear_2 = nn.Sequential(
            nn.Linear(self.hidden_dim, output_dim),
            create_activation(options.Network.linear_activation, output_dim),
            create_dropout(options.Network.dropout)
        )
