...
class BranchLinear(nn.Module):
    ...
    def __init__(
        ...
        self.skip_connections = options.Network.skip_connections
        ...
        if batch_norm:
            self.output_norm = nn.BatchNorm1d(self.hidden_dim)
        else:
            self.output_norm = nn.Identity()

        self.output_layer = nn.Linear(self.hidden_dim, num_outputs)
