...
class EmbeddingStack(nn.Module):
    ...
    @staticmethod
    def create_embedding_layers(options: DotDict, input_dim: int) -> List[BasicBlock]:
        ...
        embedding_layers = [create_linear_block(
            options,
            input_dim,
            options.Network.initial_embedding_dim,
            options.Network.initial_embedding_skip_connections
        )]
        current_embedding_dim = options.Network.initial_embedding_dim

        # Keep doubling dimensions until we reach the desired latent dimension.
        for i in range(options.Network.num_embedding_layers):
            next_embedding_dim = 2 * current_embedding_dim
            if next_embedding_dim >= options.Network.hidden_dim:
                break
