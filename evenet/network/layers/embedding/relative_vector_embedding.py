...
from evenet.network.layers.embedding_stack import EmbeddingStack
...
class RelativeVectorEmbedding(nn.Module):
    ...
    def __init__(self, options: DotDict, input_dim: int):
        super(RelativeVectorEmbedding, self).__init__()

        self.input_dim = input_dim
        self.hidden_dim = options.Network.hidden_dim
        self.mask_sequence_vectors = options.Training.mask_sequence_vectors

        self.shared_embedding_stack = EmbeddingStack(options, input_dim)
        self.shared_embedding_norm = nn.LayerNorm(self.hidden_dim)

        self.query_embedding = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.key_embedding = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.value_embedding = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.attention_scale = np.sqrt(self.hidden_dim)

        self.output = create_linear_block(options, self.hidden_dim, self.hidden_dim, options.Network.skip_connections)
    ...
    def forward(self, vectors: Tensor, time: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        attention_mask |= torch.eye(attention_mask.shape[0], dtype=attention_mask.dtype,
                                    device=attention_mask.device).unsqueeze(-1)
