...
from evenet.network.utilities import masked_log_softmax
from evenet.network.layers.stacked_encoder import StackedEncoder
from evenet.network.layers.branch_linear import BranchLinear
from evenet.network.symmetric_attention import SymmetricAttentionSplit, SymmetricAttentionFull
...
class BranchDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.combinatorial_scale = options.Training.combinatorial_scale

        # Each branch has a personal encoder stack to extract particle-level data
        self.encoder = StackedEncoder(
            options,
            options.Network.num_branch_embedding_layers,
            options.Network.num_branch_encoder_layers,
            conditioned=True
        )

        # Symmetric attention to create the output distribution
        attention_layer = SymmetricAttentionSplit if options.Network.split_symmetric_attention else SymmetricAttentionFull
        ...
        self.detection_classifier = BranchLinear(options, options.Network.num_detector_layers)

        self.num_targets = len(self.attention.permutation_group)
        self.permutation_indices = self.attention.permutation_indices

        # TODO: Inspect batch size
        self.padding_mask_operation = self.create_padding_mask_operation(options.Dataset.batch_size)
    ...
    def forward(
        ...
        encoded_vectors, particle_vector = self.encoder(event_vectors, padding_mask, sequence_mask, cond_vector)
