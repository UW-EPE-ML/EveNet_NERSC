...
from evenet.network.symmetric_attention.symmetric_attention_base import SymmetricAttentionBase
from evenet.network.layers.linear_block.masking import create_masking
from evenet.network.utilities.linear_form import create_symmetric_function
...
class SymmetricAttentionSplit(SymmetricAttentionBase):
    def __init__(self,
        ...
        self.encoders = nn.ModuleList([
            StackedEncoder(
                options,
                options.Network.num_jet_embedding_layers,
                options.Network.num_jet_encoder_layers,
                conditioned = True
            )
        ...
        self.linear_layers = nn.ModuleList([
            nn.Linear(options.Network.hidden_dim, self.attention_dim, bias=True)
            for _ in range(degree)
        ])
    ...
    def forward(self, x: Tensor, padding_mask: Tensor, sequence_mask: Tensor, cond_vector: Tensor) -> Tuple[Tensor, List[Tensor]]:
        ...
        for encoder, linear_layer in zip(self.encoders, self.linear_layers):
            ...
            # y: [T, B, D]
            # ------------------------------------------------------
            y, daughter_vector = encoder(x, padding_mask, sequence_mask, cond_vector)
