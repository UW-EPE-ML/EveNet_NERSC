...
*The exact modules that are instantiated depend on the `include_*` flags passed to the model constructor.*【F:evenet/network/evenet_model.py†L26-L316】
...
* **Sequential features** (`self.sequential_normalizer`) aggregate per-source masks/means/stds using
  ...
  inverse-CDF transforms via `sequential_inv_cdf_index`.【F:evenet/network/evenet_model.py†L57-L105】
* **Global features** (`self.global_normalizer`) default to identity statistics when no global inputs
  are declared, ensuring deterministic scaling for conditioning vectors.【F:evenet/network/evenet_model.py†L78-L105】
* **Invisible channels** (`self.invisible_normalizer`) are created only when neutrino generation is
  ...
  the PET attention map.【F:evenet/network/evenet_model.py†L90-L121】【F:evenet/network/evenet_model.py†L389-L418】【F:evenet/network/evenet_model.py†L395-L404】
* **Count features** (`self.num_point_cloud_normalizer`) normalize multiplicity inputs when global or
  point-cloud generation is active.【F:evenet/network/evenet_model.py†L107-L112】

The forward pass applies these normalizers to sequential tracks, invisible candidates, global
conditions, and counts before entering the embedding blocks.【F:evenet/network/evenet_model.py†L405-L430】
...
invisible channels (`x_invisible` with mask), as documented in the forward docstring.【F:evenet/network/evenet_model.py†L323-L387】
...
deterministic heads and diffusion decoders.【F:evenet/network/evenet_model.py†L123-L136】 Its behavior is
...
【F:share/network/pretrain.yaml†L7-L28】

During diffusion schedules, selective masking (`global_feature_mask`) zeroes out unused conditions for
reconstruction heads, while deterministic schedules keep all features active.【F:evenet/network/evenet_model.py†L471-L537】
...
task heads and diffusion decoders.【F:evenet/network/evenet_model.py†L138-L156】【F:share/network/pretrain.yaml†L16-L41】
...
* **Neutrino schedule:** concatenates normalized invisible features, enforcing visibility-aware
  masking so visible queries cannot attend to invisible keys.【F:evenet/network/evenet_model.py†L471-L537】【F:evenet/network/evenet_model.py†L507-L637】
...
heads.【F:evenet/network/evenet_model.py†L158-L548】 Major configuration knobs reside under
`Body.ObjectEncoder`, covering hidden size, positional embedding width, attention head count, encoder
layers, and dropout/skip-connection toggles.【F:share/network/pretrain.yaml†L43-L58】

The encoder returns per-object embeddings, a conditioned global representation, and an event token
used by classification, regression, assignment, and segmentation modules.【F:evenet/network/evenet_model.py†L539-L599】
...
* Instantiated when `include_classification` is true. Accepts object embeddings and predicts per-event
  class logits, optionally attending to the event token.【F:evenet/network/evenet_model.py†L175-L187】【F:evenet/network/evenet_model.py†L582-L588】
* Tuned via `Classification` configuration: depth (`num_classification_layers`), hidden width (`hidden_dim`),
  dropout, skip connections, and multi-head attention count.【F:share/network/pretrain.yaml†L60-L70】

### Regression Head

* Enabled by `include_regression`. Operates on the event token and rescales predictions using the
  regression mean/std entries from the normalization dictionary.【F:evenet/network/evenet_model.py†L189-L199】【F:evenet/network/evenet_model.py†L590-L594】
* Controlled by `Regression` settings (layer count, hidden size, dropout, skip).【F:share/network/pretrain.yaml†L72-L81】
...
* Built with `SharedAssignmentHead` when `include_assignment` is set. It consumes embeddings, global
  ...
  respecting physics symmetries provided in `event_info`.【F:evenet/network/evenet_model.py†L204-L237】【F:evenet/network/evenet_model.py†L554-L569】
* Configured via the `Assignment` block, which exposes feature dropout, transformer depth, symmetric
  attention splitting, and event-token encoding options.【F:share/network/pretrain.yaml†L83-L109】

### Segmentation Head

* Available when `include_segmentation` is true. Consumes object memory and the event token to predict
  mask logits and optional auxiliary outputs for multiple queries.【F:evenet/network/evenet_model.py†L296-L310】【F:evenet/network/evenet_model.py†L570-L581】【F:evenet/network/evenet_model.py†L643-L657】
* Tied to `Segmentation` hyperparameters: projection dimension (shared with mask width), attention heads,
  transformer depth, number of queries, intermediate returns, and normalization order.【F:share/network/pretrain.yaml†L111-L122】
...
constructor.【F:evenet/network/evenet_model.py†L312-L316】【F:evenet/network/evenet_model.py†L463-L600】
...
1. **Global generation (`include_global_generation`):**
   * Uses `GlobalCondGenerationHead` to denoise multiplicities plus selected global targets, conditioned
     on the embedded global vector and class labels.【F:evenet/network/evenet_model.py†L239-L255】【F:evenet/network/evenet_model.py†L431-L454】
   * Hyperparameters: residual block depth (`num_resnet_layers`), transformer layers, hidden/residual
     width, stochastic depth dropout, and activation.【F:share/network/pretrain.yaml†L124-L134】
   * Inputs are normalized counts concatenated with target globals; outputs supply predicted and ground
     truth vectors for diffusion losses.【F:evenet/network/evenet_model.py†L431-L454】
...
2. **Reconstruction (visible point cloud) generation (`include_point_cloud_generation`):**
   * `EventGenerationHead` consumes PET embeddings, global context, normalized counts, and diffusion time.
     ...
     via `reco_attn_mask` to gradually expand receptive fields.【F:evenet/network/evenet_model.py†L258-L274】【F:evenet/network/evenet_model.py†L471-L537】【F:evenet/network/evenet_model.py†L603-L622】
   * Config is shared under `ReconGeneration`: projection dimension, layer depth, head count, dropout,
     ...
     `time_masking` to control which features are diffused at each curriculum stage.【F:share/network/pretrain.yaml†L136-L146】【F:evenet/network/evenet_model.py†L480-L517】
...
3. **Neutrino (invisible) generation (`include_neutrino_generation`):**
   * Another `EventGenerationHead` targets invisible features, optionally applying positional encodings
     ...
     denoising.【F:evenet/network/evenet_model.py†L276-L293】【F:evenet/network/evenet_model.py†L507-L641】
   * Controlled via `TruthGeneration`: hidden size, transformer depth, head count, dropout, layer scaling,
     ...
     dimensionality before slicing out invisible predictions.【F:share/network/pretrain.yaml†L148-L160】【F:evenet/network/evenet_model.py†L90-L121】【F:evenet/network/evenet_model.py†L618-L641】
...
forward diffusion process controlled by `progressive_params` (noise probability and attention masking).【F:evenet/network/evenet_model.py†L312-L641】

Additional helper APIs (`predict_diffusion_vector`) reuse the same normalization, embedding, and head
logic for inference-time sampling in global, reconstruction, or neutrino modes.【F:evenet/network/evenet_model.py†L659-L800】
...
flags are disabled.【F:evenet/network/evenet_model.py†L643-L657】
...
* **Instantiation:** Each head/body block is created only if its associated `include_*` flag is true, so
  YAML configurations can be shared across tasks even when certain heads are inactive.【F:evenet/network/evenet_model.py†L46-L310】
* **Normalization dictionaries:** The constructor stitches together per-input normalization stats and
  ...
  regardless of which schedules execute.【F:evenet/network/evenet_model.py†L57-L121】【F:evenet/network/evenet_model.py†L405-L430】
* **Progressive schedules:** `self.schedule_flags` ties these booleans to the runtime loop, enabling
  ...
  advanced scheduling strategies without changing the model definition.【F:evenet/network/evenet_model.py†L312-L637】
